        ...
        <q-card-section>
            <q-form @submit="onSubmit" class="q-gutter-md" ref="form">
                <div class="providers row justify-center q-gutter-x-md">
                    <q-btn v-for="provider of providers" :key="provider.label" :icon="provider.icon"
                        :label="provider.label" rounded no-caps/>
                </div>
                <div class="separator row items-center gap-2" v-if="providers">
                    <q-separator size="2px" class="flex-grow"/>
                    <span>or</span>
                    <q-separator size="2px" class="flex-grow"/>
                </div>
                <div class="manual column q-gutter-y-md">
                    <q-input v-for="field in modelValue" :key="field.label" v-model="field.model" :type="field.type"
                        :label="field.label" :rules="field.rules" hide-bottom-space outlined>
                        <template v-slot:prepend v-if="field.prependIcon">
                            <q-icon :name="field.prependIcon" />
                        </template>
                        <template v-slot:append v-if="field.password">
                            <q-icon :name="field.type === 'password' ? 'visibility_off' : 'visibility'" flat dense
                ...
                <div>
                    <q-btn 
                        ...
                        type="submit" color="primary" no-caps class="w-full" push 
                        :loading="loading"
                    >
